...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.q_proj = nn.Linear(config.n_embd, config.n_head * config.head_dim, config.bias)
        self.k_proj = nn.Linear(config.n_embd, config.n_kv_head * config.head_dim, config.bias)
        self.v_proj = nn.Linear(config.n_embd, config.n_kv_head * config.head_dim, config.bias)
        self.o_proj = nn.Linear(config.n_head * config.head_dim, config.n_embd, config.bias)
    ...
    def forward(self, x, kv_cache = None, attn_mask = None): 
        ...
        #causal mask for not looking at the
        #att = att.masked_fill(self.tril_mask[:, :, total_t - current_t : total_t, :total_t] == 0, float('-inf')) 
        causal = torch.ones(current_t, total_t, device=att.device).tril(diagonal=total_t - current_t)
        att = att.masked_fill(causal == 0, float('-inf'))
        att = F.softmax(att, dim=-1)

        y = att @ v # (b, n_head, t, t) x (b, n_head, t, head_size) -> (b, n_head, t, head_size)
        y = y.transpose(1, 2).contiguous().view(b, current_t, self.n_head * self.head_dim) #re-assemble all head outputs side by side
...
class MLP(nn.Module): 
    def __init__(self, config):
        ...
        self.gate_proj = nn.Linear(config.n_embd, config.intermediate_size, bias = config.bias) #the gate / judge matrix, seperate learned questions
        self.down_proj = nn.Linear(config.intermediate_size, config.n_embd, bias = config.bias) 

    def forward(self, x):
        x = self.down_proj(F.silu(self.gate_proj(x)) * self.up_proj(x))  #silu is x * sigmoid(x)
        return x
