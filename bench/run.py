...
model.generate(prompt_ids[:, :16], 5)  # warmup (first calls pay one-time compile/alloc cost)
...
for L in PROMPT_LENGTHS:
    ...
    for _ in range(N_RUNS):
        ...
        model.generate(prompt_ids[:, :L], N_NEW) #generate 50 tokens, greedy (do_sample=False)
